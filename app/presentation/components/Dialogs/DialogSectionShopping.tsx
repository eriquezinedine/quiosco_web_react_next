...
const DialogSectionShopping = () => {
  const { enabledShopping: enabled, toggleShopping } = useUiStore();
  const {
    orders,
    cleanStore,
    totalOrders,
  ...
  return (
    <div className='duration-300 fixed top-0 bottom-0 ring-0 left-0 bg-black h-screen w-screen bg-opacity-80	flex justify-start '>
      <div className='bg-gray-100 w-zineWidth h-full'>
        ...
        <div
          className='  w-full h-full px-4 py-2 flex flex-col justify-between '
          style={{ height: '16%' }}
        >
          ...
          <div
            className='bg-slate-700 text-center font-bold text-2xl py-4 rounded-2xl text-white cursor-pointer '
            onClick={handleOrder}
          >
            {status === StatusOrder.Loading ? <Loading /> : 'Crear orden'}
          </div>
        </div>
      </div>
    </div>
