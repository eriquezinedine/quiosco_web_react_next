...
const DialogSelectionTable: FC<DialogSelectionTableProp> = ({ onClosed }) => {
  ...
  const getSelected = (isSelected: boolean, isEnabled : boolean) => {
    const style = isSelected ? ' border-black' : 'border-transparent';

    return `border-4 size-16 rounded-lg ${isEnabled ? 'bg-slate-700': 'bg-red-700' } m-2 text-white flex items-center justify-center ${style} cursor-pointer`;
  };
      ...
      >
        <div className='bg-gray-100  rounded-2xl '>
          <div className='flex  justify-between items-center px-2 mt-2'>
            <h3 className='font-bold text-2xl text-center mr-4'>
              Selecciona una mesa
            </h3>
            <IoMdClose
              onClick={() => {
          ...
          <div className='flex flex-wrap items-start justify-center py-2 '>
            {tables.map((table, index) => (
              <div
                ...
                className={getSelected(table.id === itemTable?.id,table.enabled )}
              >
                {table.number}
              </div>
            ))}
